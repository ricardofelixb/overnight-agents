...
Run definitive validation in the foreground with a timeout long enough for the entire command. Read its complete output and use its own exit code as the only pass/fail result. Never background it, pipe it through `tail`, poll it with `ps` or `pgrep`, or infer success from a monitoring command. A non-zero exit code is a failed validation even if another command succeeds later. Do not edit `organization.md`; the controller records the selected marker only after it has accepted and published the completed slice. Do not report success or end the turn until the definitive command has exited successfully and its output has been reviewed.
