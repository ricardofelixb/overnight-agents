#!/usr/bin/env python3
"""Install or remove the native macOS schedule for the code simplifier."""
...
LABEL = "com.overnight-agents.code-simplifier"
...
def configured_schedule(config_path: Path) -> str:
    matches = re.findall(r'^SCHEDULE=["\']([^"\']+)["\']\s*$', config_path.read_text(), re.MULTILINE)
    if len(matches) != 1:
        raise ValueError("config.sh must contain exactly one quoted SCHEDULE assignment")
    return matches[0]
...
def calendar_intervals(schedule: str) -> list[dict[str, int]]:
    ...
    if len(fields) != 5 or fields[2:] != ["*", "*", "*"]:
        raise ValueError("launchd migration supports daily minute/hour schedules only")
...
def definition(script_dir: Path, schedule: str) -> dict[str, Any]:
    ...
    return {
        "Label": LABEL,
        "ProgramArguments": ["/bin/bash", str(script_dir / "simplify.sh")],
...
def main() -> int:
    ...
    try:
        ...
        if args.uninstall:
            ...
            return 0
        schedule = configured_schedule(script_dir / "config.sh")
        value = definition(script_dir, schedule)
        ...
        legacy = run(["crontab", "-l"], check=False)
        if "code-simplifier/simplify.sh" in legacy.stdout:
            print("warning: legacy cron entry remains; the shared lock prevents duplicate runs")
        return 0
